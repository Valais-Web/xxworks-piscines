...
export const Route = createFileRoute("/equipements")({
  head: () => ({
    ...
    scripts: [
      { type: "application/ld+json", children: JSON.stringify(serviceJsonLd({ serviceType: "Vente et installation d'équipements de piscine", name: "Équipements de piscine", description: DESC, path: PATH })) },
      { type: "application/ld+json", children: JSON.stringify(faqJsonLd(FAQ)) },
      { type: "application/ld+json", children: JSON.stringify(breadcrumbJsonLd([{ name: "Accueil", path: "/" }, { name: "Équipements", path: PATH }])) },
    ],
  }),
  component: Page,
});
